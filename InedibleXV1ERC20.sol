...
contract InedibleXV1ERC20 is IUniswapV2ERC20 {
    ...
    string public constant symbol = "ineX-V2";
    ...
    // Added by Inedible
    function claimFees(address _user) public {
        // Div buffer is because cumulative fees is based on a full token value.
        ...
        lastUserCumulative[_user] = cumulativeFees;
        if (feeAmount > 0) {
            _transfer(address(1), address(this), feeAmount.add(_unclaimed));
            burn(_user);
        }
    }

    // Virtual function to be called on the V2Pair contract.
    function burn(address _user) public returns (uint amount0, uint amount1) {}
